...
<x-app-layout>
    <x-form-card :title="'Form Edit Jadwal Menu'" :backLink="route('jadwal-menus.index')">
        @if ($errors->any())
        ...
        <form action="{{ route('jadwal-menus.update', $jadwalMenu) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select name="sppg_id" id="sppg_id" class="form-control" required>
                        ...
                        @foreach($sppgs as $sppg)
                            <option value="{{ $sppg->id }}" {{ old('sppg_id', $jadwalMenu->sppg_id) == $sppg->id ? 'selected' : '' }}>
                                {{ $sppg->nama_dapur }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="date" name="tanggal_mulai" id="tanggal_mulai" class="form-control" value="{{ old('tanggal_mulai', $jadwalMenu->tanggal_mulai?->format('Y-m-d')) }}">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="tanggal_selesai">Tanggal Selesai</label>
                        <input type="date" name="tanggal_selesai" id="tanggal_selesai" class="form-control" value="{{ old('tanggal_selesai', $jadwalMenu->tanggal_selesai?->format('Y-m-d')) }}">
                    </div>
                </div>
            </div>
            ...
            <div class="accordion accordion-flush" id="accordionDays">
                ...
                @foreach($hariOptions as $hariKey => $hariLabel)
                    @php
                        // Get jadwal for this day if exists
                        $jadwalHari = $jadwalMenus->get($hariKey);
                        $selectedMenus = [];
                        if ($jadwalHari) {
                            foreach($jadwalHari->details as $detail) {
                                $selectedMenus[$detail->kategori_menu_id] = $detail->menu_id;
                            }
                        }
                    @endphp
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="menus[{{ $hariKey }}][{{ $kategori->id }}]" id="menu_{{ $hariKey }}_{{ $kategori->id }}" class="form-control" required>
                                            ...
                                            @foreach($kategori->menus as $menu)
                                                <option value="{{ $menu->id }}" {{ old("menus.{$hariKey}.{$kategori->id}", $selectedMenus[$kategori->id] ?? '') == $menu->id ? 'selected' : '' }}>
                                                    {{ $menu->nama_menu }}
                                                    @if($menu->deskripsi)
