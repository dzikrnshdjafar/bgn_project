...
<x-app-layout>
    <x-form-card :title="'Form Ubah Siswa'" :backLink="route('siswas.index')">
        @if ($errors->any())
        ...
        <form action="{{ route('siswas.update', $siswa->nis) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="form-group">
                <label for="nis">NIS</label>
                <input type="text" name="nis" class="form-control" id="nis" value="{{ old('nis', $siswa->nis) }}" required>
            ...
            <div class="form-group">
                ...
                @if(auth()->user()->hasRole('Operator Sekolah') && count($sekolahs) == 1)
                    ...
                    <small class="form-text text-muted">Anda hanya dapat mengedit siswa di sekolah Anda sendiri.</small>
                @else
            ...
            <div class="form-group">
                ...
                <select name="makanan_kesukaan_id" class="form-control" id="makanan_kesukaan_id">
                    ...
                    @foreach ($makanans as $makanan)
                        <option value="{{ $makanan->id }}" {{ old('makanan_kesukaan_id', $siswa->makanan_kesukaan_id) == $makanan->id ? 'selected' : '' }}>
            ...
            <button type="submit" class="btn btn-primary mt-3">Ubah Siswa</button>
        </form>
    </x-form-card>
</x-app-layout>
